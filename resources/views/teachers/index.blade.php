...
<div class="tlist-hero">
    ...
    <div class="filter-bar">
        <a href="{{ route('teachers') }}" class="filter-chip {{ !request('subject') ? 'active' : '' }}">All Teachers</a>
        ...
        @forelse($teachers as $teacher)
            <div class="t-prof-card reveal">
                <div class="t-prof-top">
                    ...
                    @if($teacher->subjects)
                        <div class="t-tags">
                            @foreach(array_slice($teacher->subjects, 0, 3) as $subject)
                                <span class="t-tag">{{ $subject }}</span>
                            @endforeach
                        </div>
                    @endif
                </div>
                ...
                <div class="t-prof-body">
                    <div class="t-meta">
                        <span class="t-meta-item">
                            <i class="fas fa-briefcase"></i>{{ $teacher->experience_years }} yrs exp.
                        </span>
                        @if($teacher->nationality)
                            <span class="t-meta-item">
                                <i class="fas fa-globe"></i>{{ $teacher->nationality }}
                            </span>
                        @endif
                        <span class="t-meta-item">
                            <i class="fas fa-video"></i>{{ $teacher->total_sessions }} sessions
                        </span>
                    </div>

                    <div class="t-rate">
                        <span class="t-rate-num">${{ number_format($teacher->hourly_rate, 0) }}</span>
                        <span class="t-rate-unit">/ hour</span>
                    </div>
                </div>

                <div class="t-prof-foot">
                    <a href="{{ route('teachers.show', $teacher) }}" class="t-btn-main">
                        View Profile
                    </a>
                    @auth
                        @if(auth()->user()->isStudent())
                            <a href="{{ route('student.bookings.create', ['teacher_id' => $teacher->id]) }}" class="t-btn-ghost" title="Book session">
                                <i class="fas fa-calendar-plus"></i>
                            </a>
                        @endif
                    @else
                        <a href="{{ route('register', 'student') }}" class="t-btn-ghost" title="Book session">
                            <i class="fas fa-calendar-plus"></i>
                        </a>
                    @endauth
                </div>
